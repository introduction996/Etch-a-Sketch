...
function openOptionsColor() {
  if (toolsOpen && border && borderRadius && width && height) {
    console.log(
      "%c color options already opened!",
...
function draw(e) {
  ...
  if (currentMode == "draw" && randomColorSelected) {
    ...
    randomColorButton.style.backgroundColor =
      colors[generateRandomIndex(colors.length - 1)];
    e.target.style.backgroundColor =
      getComputedStyle(randomColorButton).backgroundColor;
...
function generateCanvas(size) {
  ...
  if (size > 50) {
    ...
    for (let i = 1; i <= size * size; i++) {
      ...
      const sizeOfOneDiv = (canvas.offsetWidth - 10) / size; // i have to subtract the additional margin, border and padding off the element with this method
